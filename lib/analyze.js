...
export function analyze(parsed) {
  ...
  // a command that errored then got retried is a *retry loop*, not wasteful
  // re-derivation — don't also list it under redundant work (double-reporting
  // reads as a false positive). the retry section already owns it.
  const retrySigs = new Set(retryChains.map((c) => `${c.name}::${c.target}`));
  redundant = redundant.filter((r) => !retrySigs.has(`${r.name}::${r.target}`));
