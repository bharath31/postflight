...
// Parse transcript text directly — the environment-neutral core (the browser
// playground uses this; the CLI wraps it with a file read).
export function parseTranscriptText(raw, source = "") {
  const records = [];
...
export function parseTranscript(path) {
  return parseTranscriptText(readFileSync(path, "utf8"), path);
}
